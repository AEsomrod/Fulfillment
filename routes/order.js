...
router.get("/admin_page_cart", (req, res) => {
    const showname = req.session.username
    let num = 1
    if (req.session.login && req.session.typeUser == "Admin") {
        Order.find({ orderID: req.cookies.orderID }).exec((err, doc) => {
            res.render("admin_page_cart", { showname: showname, order: doc, num: num, b:0 })
        })
    }
    else {
        res.redirect("/")
...
router.post("/admin_add_order", (req, res) => {
    Order.findOne({ orderID: req.cookies.orderID }).exec((err, doc) => {
        if (!doc) {
            let OrderID = Math.random().toString(36).substring(2)
            res.cookie("orderID", OrderID)
...
router.post("/admin_delivery", (req, res) => {
