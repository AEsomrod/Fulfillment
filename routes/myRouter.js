...
router.get("/admin_dashboard", (req, res) => {
  ...
  if(req.session.login && req.session.typeUser === 'Admin'){
    let order = 1
    User.find().exec((err,doc)=>{
      res.render("admin_dashboard", { users: doc, order: order, showname: showname})
    })
  }
  else {
    res.redirect('/')
...
router.get("/stock_dashboard", (req, res) => {
  ...
  if(req.session.login){
    ...
    /*if (req.cookies.login) {
      ...
      Stock.find().exec((err, doc) => {
        res.render("stock_dashboard", { stocks: doc, groups: doct, order: order, showname: showname })
      })
    })
    /*Stock.find().exec((err,doc)=>{
      res.render("stock", { stocks: doc, order: order, showname: showname })
...
router.get("/order_admin", (req, res) => {
  ...
  if(req.session.login && req.session.typeUser === 'Admin'){
    let order = 1
    User.find().exec((err,doc)=>{
      res.render("order_admin", { users: doc, order: order, showname: showname})
    })
  }
  else {
    res.redirect('/')
...
router.get("/user_page_addgroup", (req, res) => {
  ...
  if (req.session.login && req.session.typeUser === 'User') {
    let order = 1
    Group.find({ createdBy: showname }).exec((err, doc) => {
      res.render("user_page_addgroup", { groups: doc, order: order, showname: showname })
    })
  } 
  else {
    res.redirect("/")
...
router.get("/user_page_edit_addgroup", (req, res) => {
  ...
  if (req.session.login && req.session.typeUser === 'User') {
    let order = 1
    Group.find({ createdBy: showname }).exec((err, doc) => {
      res.render("user_page_edit_addgroup", { groups: doc, order: order, showname: showname })
    })
  } 
  else {
    res.redirect("/")
  ...
  router.get("/statusorder", (req, res) => {
    ...
    if(req.session.login && req.session.typeUser === 'Admin'){
      ...
      Order_address.find().exec((err,doc)=>{
        res.render("statusorder", { order_address: doc, order: order, showname: showname})
      })
    }
    else {
      res.redirect('/')
